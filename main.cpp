...
enum type_of_lex
{
    PROGRAM, AND, OR, NOT, TRUE, FALSE, ELSE, IF, INT, READ, WHILE, WRITE, FOR, STRING, REAL, DO, GOTO, LABEL,//17
    ...
    CONST_STR, CONST_INT, CONST_REAL, IDENTIFIER, NULLP, //43
...
void print_type(type_of_lex t)
{
    ...
    else if (t == WRITE) cout << "WRITE"; else if (t == FOR) cout << "FOR"; else if (t == STRING) cout << "STRING"; else if (t == REAL) cout << "REAL";
    ...
    else if (t == QUOTES) cout << "QUOTES"; else if (t == CONST_STR) cout << "CONST_STR"; else if (t == CONST_REAL) cout << "CONST_REAL"; else if (t == CONST_INT) cout << "CONST_INT";
...
public:
    ...
    explicit Ident(const string &n, double val):name(n), d_val(val), initialization(false), declared(false) {}
...
/*int find_ident (const string & buf)
{
    vector <Ident> :: iterator k;
    if ((k = find(table_of_ident.begin(), table_of_ident.end(), buf)) != table_of_ident.end())
        return k - table_of_ident.begin() + 1;
    else return 0;
}*/

vector <Label> table_of_label;

//Stack <Label, 100> expected_labels;
...
public:
    ...
    {}
    explicit Lexeme( double val, type_of_lex t = NULLP): type(t), number(t), d_val(val)
    {}
...
const string Lex_Scanner:: service_words[] =
        {"program", "and", "or", "not", "true", "false", "else", "if", "int", "read", "while", "write", "for", "string", "real", "do", "goto", ""};
    ...
    do {
        ...
        switch(current_state) {
            ...
            case NUMB:
                ...
                else if (c == '.') { r = (double)d; current_state = RL;}
            ...
            case RL:
                if ( isdigit (c) ) {
                    r = r + (c - '0') * pow(1e-1, after_dec);
                    after_dec++;
                }
                else {
                    file.seekg(old_pos);
                    d_value = r;
                    return Lexeme( r,CONST_REAL);
                }
                break;
...
public:
    ...
    void check_op()
    {
        ...
        } else if ((t1 == INT || t1 == REAL) && ((t2 == INT || t2 == REAL))) {
            if (op == PLUS || op == MIN || op == MUL || op == SLASH) {
                if (t1 == INT && t2 == INT) st_lex.push(INT);
                else st_lex.push(REAL);
            } else if (op == LS || op == GE || op == TWOEQUAL || op == NEQ || op == GEQ || op == LEQ) st_lex.push(INT);
            else if ((op == AND || op == OR) && (t2 == t1) && (t1 == INT)) st_lex.push(INT);
            else throw "Wrong operations with INT or REAL";
        }
    ...
    void check_id()
    {
        ...
        if (table_of_ident[number].get_declare())
        {
            switch (table_of_ident[number].get_type_of_id()) {
                ...
                case REAL_VAR: st_lex.push(REAL); break;
            }
    ...
    void eq_type()
    {
       ...
       if (((first == INT) || (first == REAL)) && (sec == INT || sec == REAL)) {st_lex.push(first);}
...
void Parser::D1()
{
    ...
    else if (l_type == REAL)
    {
        do {
            ...
            if (table_of_ident[number].get_declare()) { string err = "Variable already declared!"; throw err; }
            table_of_ident[number].set_declare();
            table_of_ident[number].set_type_of_id(REAL_VAR);
            gl();
            if (l_type == EQ){
                table_of_ident[number].set_init();
                gl();
                int val = 1;
                if (l_type == MIN) {gl(); val = -1;}
                table_of_ident[number].set_value(curr_lex.d_val * val);
                if (l_type != CONST_REAL) throw curr_lex;
                gl();
            }
        } while (l_type == COMMA);
        if (l_type != SEMICOLON) throw curr_lex;
...
void Parser::E1()
{
    ...
    T();
    while (l_type == PLUS || l_type == MIN || l_type == OR)
    {
        flag_for_id = false;
        st_lex.push(l_type);
        gl();
...
int main(int args, char const *argv[]) {
        ...
        try {
            ...
            cout << "Syntax error - " << lex << "In line - " << parser.get_line() << '\n';
        }
        catch (string str) {
            cout <<"Syntax error - " << str << '\n';
        }
        catch (const char* str) {
            cout << "Error - " << string(str) << '\n';
        }
        vector <Ident> :: iterator k;
        k = table_of_ident.begin();
        while ( k != table_of_ident.end())
